...
a.  The reason why athlete names didn't match across the two dataa frames is that in one data frame names are in UPPER CASE and in the other they're in Title Case.
    Update the 2020 data frame to make `name` all upper case.
    Display the first 10 rows of `team_usa_2020` with upper cse `name`s.
