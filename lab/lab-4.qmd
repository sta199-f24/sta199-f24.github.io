...
In this lab you'll both review topics you've worked with in previous labs and practice importing data and dive into the concepts of data types and classes.
...
For this part you'll work with data from the rosters of Team USA from 2020 and 2024 Olympics.
...
Your goal is to answer questions around athletes that competed in both games and only one of the games.

### Question 1

a.  Read data in from the two sheets of `team-usa.xlsx` as two separate data frames called `team_usa_2020` and `team_usa_2024`.

::: callout-tip
You can see the names of the sheets in the screenshots in @fig-olympics-sheets or you can also use the `excel_sheets()` function.
...
a.  The reason why athlete names didn't match across the two dataa frames is that in one data frame names are in UPPER CASE and in the other they're in Title Case. Update the 2020 data frame to make `name` all upper case. Display the first 10 rows of `team_usa_2020` with upper cse `name`s.
...
d.  Calculate the proportions of individuals in this sample who who think medical marijuana should remain against the law, should be made legal, and who are not sure
...
a.  Create a visualization that can be used to evaluate the relationship between `age` and `opinion` on legalizing medical marijuana in North Carolina based on this survey's results.
    ...
    ::: callout-tip
    Your visualization should display the proportions you calculated in Question 8d.
    :::
...
In this part you'll work with one of the most basic, most overused datasets in R: `mtcars`.
The data in this dataset come from the 1974 *Motor Trend* US magazine (so, yes, they're old!) and provide information on fuel efficiency as well as other characteristics of the cars.
...
Your task is to make your plot from Question 1b as ugly and as ineffective as possible.
...
-   a list of the at least 5 updates you've made to your plot from Question 1b, and
