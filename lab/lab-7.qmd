...
b.  Fit a logistic regression model predicting `spam` from `exclaim_mess` (the number of exclamation points in the email message).
    Then, display the tidy output of the model.

c.  Using this model and an R function like `predict()` or `augment()`, predict the probability the email is spam if it contains 10 exclamation points.

## Question 2

a.  Fit another logistic regression model predicting `spam` from `exclaim_mess`, `winner` (indicating whether “winner” appeared in the email), and `urgent_subj` (whether the word "urgent" is in the subject of the email).
    Then, display the tidy output of the model.
...
a.  Fit another logistic regression model predicting `spam` from `exclaim_mess` and another variable you think would be a good predictor.
