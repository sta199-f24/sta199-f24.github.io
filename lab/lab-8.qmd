...
e.  Make predictions for the testing data. Then, in a single pipeline, calculate the false positive and negative rates. Make sure that your answer makes it clear which rates are which.

and draw the ROC curve, and calculate the AUC (area under the curve).

## Question 2

a.  Fit another model to the training data predicting whether a reservation is canceled from the variables you used in Question 8 plus one other predictor.
State the new predictor and why you chose it.

b.  Make predictions for the testing data. Then, in a single pipeline, calculate the false positive and negative rates. Make sure that your answer makes it clear which rates are which.

## Question 3

a.  What was the name you used for the model fit object for the model you created in Question 1? What about in Question 2?
...
What is the slope of the regression line for predicting weights of lemurs (`weight_g`) from the ages of lemurs (in years) when their weight was measured (`age_at_wt_y`) and their types (`taxon`)?
Calculate and interpret a 95% bootstrap bootstrap confidence interval together with your point estimate.
Don't forget to set a seed and use 1,000 bootstrap samples (`reps = 1000`) when simulating your bootstrap distribution.
...
a. Conduct a hypothesis test to answer this question at 5% significance level. Clearly state your hypotheses in context of the data and the research question, simulate a randomization distribution, find the p-value, and make a decision on your hypotheses based on this p-value. Make sure to provide a one-sentence conclusion for your hypothesis test in context of the data and the research question. Don't forget to set a seed and use 1,000 randomization samples (`reps = 1000`) when simulating your randomization distribution.
...
c. Construct a 95% bootstrap confidence interval for the difference in means of female and male lemurs. Does it include 0? Does this align with your answer to Part (b)? Don't forget to set a seed and use 1,000 bootstrap samples (`reps = 1000`) when simulating your bootstrap distribution.
