...
-   What combinations of data types have we seen so far?
...
-   We estimate the parameters using *maximum likelihood* (don't worry about it) to get the "best fitting" S-curve;
...
#legend("topleft", paste("p =", p_thresh), bty = "n", cex = 2)
points(X, pch = 19, 
     col = c(rep("orange", n), rep("blue", n)))
```

-   if new $(x_1,\,x_2)$ below, then $\text{Prob}(y=1)\leq p^*$, so predict $\widehat{y}=0$ for the new person;
-   if new $(x_1,\,x_2)$ above, then $\text{Prob}(y=1)> p^*$, so predict $\widehat{y}=1$ for the new person.
